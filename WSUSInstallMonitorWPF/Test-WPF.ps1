...
Param($path)
    ...
    write-host "test refresh"
   $synchash.listView.Dispatcher.Invoke([action]{$synchash.listView.Items.Refresh(),"Normal"})
  #$synchash.listView.items.Refresh()
  $synchash.host.runspace.events.generateEvent("ListViewChanged",$synchash.listView,$null,"Changed")
...
Register-EngineEvent -SourceIdentifier "ListViewChanged" -Action {$Global:x.host.UI.Write("Event Happened!")}
